...
public class LinkedSubmap extends Submap {
  private static final Logger LOGGER = LogManager.getFormatterLogger(RetailSubmap.class);
  public final int idCut;
  public final int refCut;
  ...
  private final ArrayList<LinkedAsset> linkedAssets;
  public LinkedSubmap(final int idCut, final int refCut, final NewRootStruct newRoot, final Vector2f screenOffset, final CollisionGeometry collisionGeometry) {
    this.idCut = idCut;
    this.refCut = refCut;
  ...
  public void loadEnv(final Runnable onLoaded) {
    LOGGER.info("Loading submap cut %d environment", this.refCut);

    final IntRef drgnIndex = new IntRef();
    final IntRef fileIndex = new IntRef();

    this.newRoot.getDrgnFile(this.refCut, drgnIndex, fileIndex);
  ...
  public void loadAssets(final Runnable onLoaded) {
    LOGGER.info("Loading linked submap cut %d assets with ref %d", this.idCut, this.refCut);

    this.theEnd_800d4bd0 = null;

    if(this.refCut == 673) { // End cutscene
    ...
    if(drgnIndex.get() == 1 || drgnIndex.get() == 2 || drgnIndex.get() == 3 || drgnIndex.get() == 4) {
      ...
      final int assetCutIndex = smapFileIndices_800f982c[this.refCut];
      final int expectedCount = assetCutIndex == 0 ? 1 : 2;

      Unpacker.loadDirectory("../assets/submap/%d".formatted(this.idCut), scripts::addAll);

      for(int i = 0; i < this.linkedAssets.size(); i++){
        this.newRoot.getDrgnFile(this.linkedAssets.get(i).cut(), drgnIndex, fileIndex);
        //Load mesh&anims
        for(int y = 0; y < 33; y++) {
          final IntRef tempIndex = new IntRef();
        ...
        //Load texture
        final FileData textureData = Unpacker.loadFile("SECT/DRGN" + (20 + drgnIndex.get()) + ".BIN/" + (fileIndex.get() + 1) + "/textures/" + linkedAssets.get(i).index());
        textures.add(textureData);
      }
      final int cutFileIndex = smapFileIndices_800f982c[this.refCut];

      this.prepareSobjs(assets,scripts,textures);
      onLoaded.run();
      ...
      if(cutFileIndex != 0) {
        ...
        final int expectedCutCount = this.refCut == 673 ? 3 : 2;
        ...
        if(this.refCut == 673) { // End cutscene, loads "The End" TIM
          loadDrgnFile(0, 7610, file -> allLoaded(cutCount, expectedCutCount, () -> {
            LOGGER.info("Submap cut %d the end texture loaded", this.refCut);
            this.theEnd_800d4bd0.setTim(new Tim(file));
          }, prepareMapAndComplete));
        }

        // File example: 7508
        LOGGER.info("Loading submap cut %d overlay model file %d", this.refCut, cutFileIndex);
        loadDrgnDir(0, cutFileIndex, files -> allLoaded(cutCount, expectedCutCount, () -> {
          LOGGER.info("Submap cut %d overlay model loaded", this.refCut);
          this.submapCutModel = new CContainer("DRGN0/" + cutFileIndex, files.get(0));
          this.submapCutAnim = new TmdAnimationFile(files.get(1));
        }, prepareMapAndComplete));

        LOGGER.info("Loading submap cut %d overlay texture and matrix file %d", this.refCut, cutFileIndex + 1);
        loadDrgnDir(0, cutFileIndex + 1, files -> allLoaded(cutCount, expectedCutCount, () -> {
          LOGGER.info("Submap cut %d overlay texture and matrix loaded", this.refCut);
  ...
  public void loadMapTransitionData(final MapTransitionData4c transitionData) {
    ...
    if(this.refCut != 0) {
      for(final SubmapWorldMapExits exit : this.submapWorldMapExits_800f7f74) {
        if(exit.submapCut_04 == this.refCut) {
  ...
  void applyCollisionDebugColour(final int collisionPrimitiveIndex, final RenderEngine.QueuedModel model) {
    for(int n = 0; n < this.submapWorldMapExits_800f7f74.length; n++) {
      ...
      if(worldMapExits.submapCut_04 == this.refCut && collisionPrimitiveIndex == worldMapExits.primitiveIndex_06) {
  ...
  @Override
  public void unload() {
    previousSubmapCut_800bda08 = this.refCut;
  ...
  public int getEncounterRate() {
    return encounterData_800f64c4[this.refCut].rate_02;
  }

  @Override
  public void generateEncounter() {
    encounterId_800bb0f8 = sceneEncounterIds_800f74c4[encounterData_800f64c4[this.refCut].scene_00][this.randomEncounterIndex()];
    battleStage_800bb0f4 = encounterData_800f64c4[this.refCut].stage_03;
  }

  @Override
  public void storeStateBeforeBattle() {
    submapCutBeforeBattle_80052c3c = this.refCut;
  }

  @Override
  public boolean isReturningToSameMapAfterBattle() {
    return submapCutBeforeBattle_80052c3c == this.refCut;
  }

  private void prepareSobjs(final List<FileData> assets, final List<FileData> scripts, final List<FileData> textures) {
    LOGGER.info("Submap cut %d preparing sobjs", this.refCut);
  ...
  private void prepareMap(final Tim submapCutTexture, final MV submapCutMatrix) {
    LOGGER.info("Submap cut %d preparing map", this.refCut);
  ...
  private void loadTextureOverrides() {
    ...
    this.sobjTextureOverrides.putAll(EVENTS.postEvent(new SubmapObjectTextureEvent(drgnBinIndex_800bc058, this.refCut)).textures);
  }
  ...
  private void loadCollisionAndTransitions() {
    ...
    final SubmapCutInfo entry = this.newRoot.submapCutInfo_0000[this.refCut];
  ...
  public void prepareEnv() {
    LOGGER.info("Submap cut %d preparing environment", this.refCut);
    ...
    final SubmapEnvironmentTextureEvent event = EVENTS.postEvent(new SubmapEnvironmentTextureEvent(drgnBinIndex_800bc058, this.refCut, this.envForegroundTextureCount_800cb580));
  ...
  private boolean FUN_800e6d58() {
    for(int i = 0; i < 45; i++) {
      if(this._800f7e58[i] == this.refCut) {
        return true;
      }
    }
        ...
        if(submapId == music.submapId_00) {
          ...
          for(int cutIndex = 0; cutIndex < music.submapCuts_04.length; cutIndex++) {
            if(submapId == 57) { // Opening (Rose intro, Dart forest, horses)
              if(music.submapCuts_04[cutIndex] != this.refCut) {
                continue;
              }
            ...
            //LAB_8001c6ac
            if(music.submapCuts_04[cutIndex] == this.refCut && (gameState_800babc8._1a4[submapMusicIndex >>> 5] & 0x1 << (submapMusicIndex & 0x1f)) != 0) {
              //LAB_8001c7c0
              musicIndex = music.musicIndex_02;
              break jmp_8001c7a0;
            }
  ...
  private int getCurrentSubmapMusic() {
    if(submapId_800bd808 == 56) { // Moon
      for(int i = 0; ; i++) {
        ...
        if(moonMusic.submapCut_00 == this.refCut) {
