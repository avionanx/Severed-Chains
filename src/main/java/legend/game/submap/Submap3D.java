...
public class Submap3D extends Submap {
  ...
  public Submap3D(final String name, final NewRootStruct newRoot, final Vector2f screenOffset, final RaycastedTrisCollisionGeometry collisionGeometry, int refCut) {
  ...
  public void loadAssets(final Runnable onLoaded) {
    ...
    if(drgnIndex.get() == 1 || drgnIndex.get() == 2 || drgnIndex.get() == 3 || drgnIndex.get() == 4) {
      ...
      scripts.addAll(Loader.loadDirectory("../assets/submap/%s/scripts".formatted(this.name)));
      ...
      for(int i = 0; i < this.linkedAssets.size(); i++){
        ...
        //Load texture
        final FileData textureData = Loader.loadFile("SECT/DRGN" + (20 + drgnIndex.get()) + ".BIN/" + (fileIndex.get() + 1) + "/textures/" + linkedAssets.get(i).index());
  ...
  @Override
  public void prepareEncounter(boolean useBattleStage) {

  }

  @Override
  public void prepareEncounter(int encounterId, boolean useBattleStage) {
